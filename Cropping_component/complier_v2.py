...
class Block(tk.Frame):
    ...
    def __init__(self, master, component_type, canvas, delete_area, image_area, info_area, param_area, block_param_list
                 , script_name, multiply, page, **kwargs):
    ...
    def grid_pos(self):
        return self.winfo_y() // self.grid_size * 10 + self.winfo_x() // self.grid_size + self.page * 100
    ...
    def update_script(self):
        ...
        # input_param.append(99)
        input_param.append(min((app.page * 100 + 99), len(self.block_param_list)))
...
class App:
    def __init__(self, root, script_name, multiply):
        ...
        # Add the buttons side by side in the frame
        self.Pre_page = tk.Button(self.nav_frame, text="Last", command=self.pre_page)
        self.Pre_page.pack(side=tk.LEFT, padx=5)

        self.Next_page = tk.Button(self.nav_frame, text="Next", command=self.next_page)
        self.Next_page.pack(side=tk.LEFT, padx=0)
    ...
    def create_palette_blocks(self):
        ...
        for text, color_tags in zip(block_texts, color_tags):
            block = Block(self.palette, text, self.canvas, self.delete_area, self.image_area ,self.info_area, self.param_area,
                          self.block_param_list, self.script_name, self.multiply, 0, bg=color_tags)
            block.pack(pady=10, padx=10)
            block.bind("<ButtonRelease-1>", self.on_palette_block_release)  # must
    ...
    def create_grid(self):
        for i in range(10):
            for j in range(10):
                ...
                self.canvas.create_text((x0 + x1) // 2, (y0 + y1 + 40 * self.multiply) // 2, text=j * 10 + i + 1 + self.page * 100, tags="grid")
    ...
    def next_page(self):
        max_page = int(len(self.block_param_list) / 100)
        ...
        for block_id, comp in enumerate(self.block_param_list):
            if int(block_id / 100) == self.page:
                ...
                new_block = Block(self.canvas, component_type, self.canvas, self.delete_area, self.image_area, self.info_area,
                                  self.param_area, self.block_param_list, self.script_name, self.multiply, self.page, bg=color)
